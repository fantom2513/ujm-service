...
export function normalizeChatAttachment(file: UploadedFile): { ok: true; text: string } | { ok: false; reason: "format" | "attachment" } {
  ...
  if (isChatDocumentFormat(format)) {
    ...
    const normalized = normalizeTextFile(file);
